...
class VolcesTTSMgr: NSObject {
    ...
    func sendText(_ text: String) {
        ...
        JLLogManager.logLevel(.DEBUG, content: "发送语音:\(text),data:\(data)")
...
extension VolcesTTSMgr: WebSocketDelegate {
    ...
    func didReceive(event: Starscream.WebSocketEvent, client: Starscream.WebSocket) {
        ...
        case let .connected(headers):
            isConnected = true
            JLLogManager.logLevel(.WARN, content: "websocket is connected: \(headers)")
...
class TTsDataSource: NSObject {
    ...
    static func initData(_ data: Data) -> TTsDataSource? {
        ...
        case 11: // 音频响应
            if instance.msgTypeSpecificFlag != 0 {
                ...
                if instance.sequenceNumber < 0 {
                    JLLogManager.logLevel(.WARN, content: "收到最后一个音频分段")
                }
            }
        ...
        default:
            JLLogManager.logLevel(.WARN, content: "未知消息类型: \(instance.msgType)")
            return nil
        }
